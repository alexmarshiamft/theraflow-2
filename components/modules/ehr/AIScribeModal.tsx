      ...
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">Ambient AI Scribe</h2>
              <p className="text-xs text-brand-600 font-medium">Zero-Retention Architecture Active</p>
            </div>
          </div>
        ...
        <div className="p-8 flex-1 overflow-y-auto">
          ...
          {state === 'idle' && (
            <div className="flex flex-col items-center justify-center text-center py-10 space-y-6">
              ...
              <div className="max-w-sm">
                <h3 className="text-xl font-bold text-gray-900 mb-2">Ready to Record</h3>
                <p className="text-gray-500 text-sm leading-relaxed">
                  Start recording your session. The audio will be transcribed, formatted into a SOAP note, and <strong className="text-gray-700">instantly deleted</strong> from memory.
                </p>
                {clientName && <p className="mt-4 text-xs font-semibold text-brand-600 bg-brand-50 py-1.5 px-3 rounded-full inline-block">Context: {clientName} - {appointmentType}</p>}
              </div>
              <div className="flex flex-col gap-3 w-full max-w-xs mt-4">
                <Button size="lg" onClick={handleStartRecording} className="bg-brand-600 hover:bg-brand-700 rounded-xl shadow-lg shadow-brand-200">
                  <Mic className="h-5 w-5 mr-2" /> Start Recording
                </Button>
                <Button size="sm" variant="ghost" onClick={handleMockDemo} className="text-gray-400 text-xs mt-4">
          ...
          {state === 'recording' && (
            <div className="flex flex-col items-center justify-center text-center py-12 space-y-8">
              <div className="relative">
                ...
                <div className="relative h-32 w-32 rounded-full bg-red-50 border-4 border-red-100 flex items-center justify-center shadow-inner">
                  <Mic className="h-12 w-12 text-red-500 animate-pulse" />
                </div>
              </div>
              <div>
                <h3 className="text-2xl font-bold text-gray-900 mb-1">{formatTime(recordingTime)}</h3>
              ...
              <Button size="lg" onClick={handleStopRecording} className="bg-gray-900 hover:bg-black text-white rounded-xl shadow-xl min-w-[200px]">
          ...
          {state === 'processing' && (
            <div className="flex flex-col items-center justify-center text-center py-16 space-y-6">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-900 mb-2">Analyzing Session...</h3>
          ...
          {state === 'review' && (
            <div className="flex flex-col h-full space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-gray-400 font-mono">Status: Audio Shredded</span>
              </div>
              <textarea
                value={generatedNote}
                onChange={(e) => setGeneratedNote(e.target.value)}
                className="flex-1 w-full p-4 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 text-gray-700 text-sm leading-relaxed font-sans min-h-[300px] resize-none"
              />
            </div>
          )}
          ...
          {state === 'error' && (
            <div className="flex flex-col items-center justify-center text-center py-16 space-y-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-900 mb-1">Recording Failed</h3>
                <p className="text-gray-500 text-sm max-w-xs mx-auto">{errorMessage}</p>
              </div>
